...
/-!
# Diagonal and unipotent elements of the general linear group

Two small constructions in `Matrix.GeneralLinearGroup`, stated over an arbitrary
commutative ring, used in Hecke-operator double-coset arguments. Material
destined for Mathlib.

* `Matrix.GeneralLinearGroup.diagonal` — the invertible diagonal matrix attached
  to a vector of units.
* `Matrix.GeneralLinearGroup.GL2.unipotent` — the `2 × 2` unipotent `!![1, t; 0, 1]`,
  together with its defining equation, its inverse, and its additive composition law.
-/
...
-- **OPEN (upstream dedup):** check whether mathlib now provides this 2×2 unipotent-matrix construction (note: it is not an AddChar).
