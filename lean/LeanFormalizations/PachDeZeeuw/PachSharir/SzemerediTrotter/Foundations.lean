...
/-!
# Szemerédi–Trotter — Foundations (shard 1 of 5)

Incidence definitions, the `lineKey` ordering, the per-line edge lists, the
straight segment `segmentArc`, the assembled drawing `stMultigraph`, the
geometry-free multiplicity bookkeeping, and the W1–W3 geometric infrastructure
(incident endpoints/angles) feeding the angle-regularity proof in the next
shard.  See the coordinator module `…PachSharir.SzemerediTrotter` for the full
overview.
-/
...
noncomputable def complexVec (p q : ℝ × ℝ) : ℂ :=
  ⟨q.1 - p.1, q.2 - p.2⟩
...
lemma not_sameRay_of_lineKey_opposite {ℓ : Set (ℝ × ℝ)} (hℓ : IsAffineLine ℓ)
...
lemma mem_edgesOnLine_lineKey_lt {P : Finset (ℝ × ℝ)} {ℓ : Set (ℝ × ℝ)}
...
lemma edgesOnLine_source_unique {P : Finset (ℝ × ℝ)} {ℓ : Set (ℝ × ℝ)}
...
lemma edgesOnLine_target_unique {P : Finset (ℝ × ℝ)} {ℓ : Set (ℝ × ℝ)}
...
lemma allEdges_getElem_inj (P : Finset (ℝ × ℝ)) (L : Finset (Set (ℝ × ℝ)))
...
noncomputable def stMultigraph_incidentOppositeEndpoint
    (P : Finset (ℝ × ℝ)) (L : Finset (Set (ℝ × ℝ)))
...
lemma stMultigraph_incidentOppositeEndpoint_ne
    (P : Finset (ℝ × ℝ)) (L : Finset (Set (ℝ × ℝ))) {p : ℝ × ℝ}
    {e : Fin (stMultigraph P L).numEdges × Bool}
    (he : e ∈ incidentEnds (stMultigraph P L) p) :
...
lemma sameRay_of_stMultigraph_incidentAngle_eq
    (P : Finset (ℝ × ℝ)) (L : Finset (Set (ℝ × ℝ))) (p : ℝ × ℝ)
...
lemma lineForEdge_eq_of_stMultigraph_incidentAngle_eq
    (P : Finset (ℝ × ℝ)) (L : Finset (Set (ℝ × ℝ)))
...
lemma incidentEnds_source_eq (P : Finset (ℝ × ℝ)) (L : Finset (Set (ℝ × ℝ)))
...
lemma incidentEnds_target_eq (P : Finset (ℝ × ℝ)) (L : Finset (Set (ℝ × ℝ)))
    {p : ℝ × ℝ} {i : Fin (stMultigraph P L).numEdges}
...
lemma allEdges_pair_mem_edgesOnLine_lineForEdge
    (P : Finset (ℝ × ℝ)) (L : Finset (Set (ℝ × ℝ)))
    (i : Fin (stMultigraph P L).numEdges) :
