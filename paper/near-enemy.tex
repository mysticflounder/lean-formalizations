...
$\E(T(G))=2n(n-1)$.
The projected set also has zero rotational energy and its set of distinct
distances is in bijection with the $\pm$-difference classes of $G$.
...
\begin{enumerate}
  ...
  \item[\rm(f)] the set of distinct distances of $T(G)$ is in bijection with
...
\begin{remark}[Why a single coincidence is not enough]
\label{rem:single}
The orthogonality relation~(ii) is only the scalar consequence of a
\emph{single} shared bisector, and on its own it does \emph{not} force the
pairs to coincide: the unit square $\{(0,0),(1,0)\}$, $\{(0,1),(1,1)\}$ has no
three collinear points and satisfies both (i) and (ii), yet the two pairs are
distinct (their common bisector is the line $x=\tfrac12$).  Rigidity instead
requires the full \emph{vector} condition $p+q=p'+q'$ (equal midpoints):
combined with~(i) it places all four points on one line, so a
no-three-collinear hypothesis forces the pairs to be equal.  The
generic-projection argument of the next section supplies exactly this upgrade,
by making the orthogonality polynomial vanish for \emph{every} projection row
simultaneously, which promotes the scalar relation to the vector identity.
\end{remark}
...
\end{align*}
Condition~(avoid) fails for this quadruple exactly at those $T$ where both
$D=0$ and $H=0$, so it suffices to show $D^2+H^2$ is not the zero polynomial;
its real zero-set then has measure zero.
We claim $D$ and $H$ cannot both vanish identically.
First, $D\equiv 0$ (as a polynomial in $T_1,T_2$) if and only if $b-a$ and
$e-c$ are linearly dependent, i.e.\ $e-c=t(b-a)$ for some $t\in\R$.
Second, writing $m=a+b-(c+e)$ and $u=b-a\ne 0$, we have
$H=\sum_k\ip{T_k}{m}\ip{T_k}{u}$, whose associated symmetric form is
$\tfrac12(mu^\top+um^\top)$; this vanishes identically if and only if $m=0$,
i.e.\ $a+b=c+e$.
If both held, then $e-c=t(b-a)$ and $a+b=c+e$: letting $M$ be the common
midpoint, $a=M-v$, $b=M+v$ with $v=\tfrac12(b-a)\ne 0$, and $c=M-w$, $e=M+w$
with $w=\tfrac{t}{2}(b-a)=tv$; thus $a,b,c$ all lie on the line $M+\R v$.
They are distinct (if $t\in\{1,-1\}$ then $\{c,e\}=\{a,b\}$, excluded), so
$G$ would contain three collinear points, contradiction.
Hence $D^2+H^2$ is a nonzero polynomial in $2d$ variables.
...
of finitely many measure-zero algebraic sets, hence nonempty (in fact dense):
a finite union of proper zero-sets of nonzero polynomials on $\R^{2d}$ has
Lebesgue measure zero and dense complement.
\end{proof}
