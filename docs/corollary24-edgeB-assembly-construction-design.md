...
| `edgeBMultigraph_multiplicity_le_M` | `hmult ≤ M` | **PROVEN (landed) — `EdgeBMultiplicity.lean`, axiom-clean** | takes point–point ≤M clause as hyp in PlanePoly rep; multiplicity = countP over `allCurveEdges`, ≤1-per-curve (M2 core, left-endpoint pivot) then ≤M by the clause; build `docs/corollary24-edgeB-multiplicity-build.md` |
| `edgeBMultigraph_wellDrawn` | `crossingCount ≤ M·|Γ|²` | **PROVEN (landed) — `EdgeBWellDrawn.lean`, axiom-clean** | takes curve–curve ≤M clause as hyp in PlanePoly rep; same-curve 0 via landed export_4b + `allCurveEdges_nodup` (NEW, design under-specified) + `goodIntervalsBundle_no_overlap` (NEW); cross-curve injects fibres into Γ×ˢΓ; build `docs/corollary24-edgeB-welldrawn-build.md` |
| `edgeBMultigraph_arcsJoinEndpoints` | `ArcsJoinEndpoints` | **PROVEN (landed) — `EdgeBArcsJoin.lean`, axiom-clean** | 2-line lift of landed `EdgeBEdge.arc_endAnchor` over the global edge index; build `docs/corollary24-edgeB-arcsjoin-build.md` |
