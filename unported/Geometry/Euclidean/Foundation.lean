...
configurations `Fin n → ℝ²`; the canonical `Erdos98.h` from
`FormalConjectures.ErdosProblems.«98»` is stated over `Finset ℝ²`. The
image-finset operation `Config.toFinset` plus the lemma `Config.toFinset_card`
(in `Erdos98Proof.Bridge`) mediate between the two shapes.
-/
...
/-- Alias for the ambient plane in Problem 98. -/
abbrev Point : Type := ℝ²
...
/-- Indexed-shape minimum distinct-distance count over general-position configurations. Bridges to the canonical `Erdos98.h` (Finset-shape) via `Bridge.hIndexed_eq_h`; equivalent on both sides via `Config.toFinset` / `Finset.equivFin`. -/
